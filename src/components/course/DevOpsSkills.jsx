...
const DevOpsSkills = (props) => {
  ...
  const filteredRoles = props.jobRoles
    ? props.jobRoles
    : JobRoles.filter(
        (role) =>
  ...
  return (
    <div className="bg-white border rounded-lg p-3 sm:p-4 md:p-6 w-full mx-auto text-black">
      <div className="mb-4">
        ...
        <h3 className=" mt-6 mb-5 text-base sm:text-lg font-semibold">
          Top Companies
        </h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
          {filteredRoles.map((role, index) => (
            <div
              key={index}
              className="flex items-center space-x-2 sm:space-x-3 p-2 rounded-md hover:bg-gray-300 transition-colors"
            >
              <Image
                src={role.logo}
                alt={role.logoAlt}
